          ...
          <scroll-view scroll-y style="height: 100%; width: 100%">
            <view class="page-box" v-if="dataList.length">
              <view class="order" v-for="(res, index) in dataList" :key="index">
                ...
                <view class="item">
                  <view class="left">
                    <image
                      src="/static/room/web1.jpg"
                      mode="aspectFill"
                    ></image>
                  </view>
                  <view class="content">
                    <view class="title u-line-2 text-bold"
                      >当前会议：{{ res.roomName }}</view
                    >
                    <view class="type">会议室简介：{{ res.roomMsg }}</view>
                    <view class="delivery-time"
                      >管理员： {{ res.roomName }}</view
                    >
                  </view>
                </view>
                <view class="bottom">
                  <view>
                    <view class="cu-avatar-group" style="padding-left: 24rpx">
                      <view
                        ...
                        :key="index"
                        :style="[
                          { backgroundImage: 'url(' + avatar[index] + ')' },
                        ]"
                      ></view>
                    </view>
                    <text class="text-blue text-shadow onLineCount"
                      >在线人数:12人</text
                    >
                  </view>
                  ...
                  <view class="btnBox">
                    <view
                      @click="goClass(res, userInfo.id)"
                      class="evaluate btn btn1"
                      >进入</view
                    >
                    <!-- <view class="exchange btn">分享课程</view> -->
                    <view @click="goRemove(res.id)" class="evaluate btn btn2"
                      >移除</view
                    >
                  </view>
                </view>
            ...
            <view class="page-box" v-else>
              <view>
                <view class="centre">
                  <image
                    src="http://cdn.zhoukaiwen.com/noData1.png"
                    mode="widthFix"
                  ></image>
                  <view class="explain">
                    暂无会议室信息
